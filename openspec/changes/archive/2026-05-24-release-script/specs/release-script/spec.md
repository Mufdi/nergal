...
These belong to a future OpenSpec change `release-ci-signed`.

#### Scenario: Script terminates after push with reminder
- **WHEN** all steps complete successfully
- **THEN** the script SHALL exit 0 immediately after `git push origin v<new>`
- **AND** SHALL print: "Build artifacts and GitHub release remain manual until Change B (`release-ci-signed`)."
