...
## Requirements
### Requirement: New CLI subcommand `nergal post-session`
The system SHALL add a new top-level subcommand to the `nergal` CLI binary: `nergal post-session`. The subcommand SHALL acquire a global advisory file lock at `~/.config/nergal/post-session.lock` using `fs2::FileExt::try_lock_exclusive`, drain pending markers from `~/.config/nergal/pending-mocs/`, and exit. If the lock cannot be acquired (sibling already running), the subcommand SHALL exit 0 immediately with no work.
...
The system SHALL drop a marker file at `~/.config/nergal/pending-mocs/<session_id>.json` on every event that should produce a session MOC. Triggers MUST be:
...
- **THEN** the dispatcher SHALL write `~/.config/nergal/pending-mocs/S.json`
- **AND** spawn `nergal post-session` detached
...
If the recovery run fails (logged in `~/.config/nergal/logs/post-session.log`), the system SHALL emit a louder toast pointing at the log path.

#### Scenario: Stale markers on launch

- **WHEN** Nergal starts
- **AND** `~/.config/nergal/pending-mocs/` contains a marker file with `created_at` older than 10 minutes
...
The runner SHALL log every step to `~/.config/nergal/logs/post-session.log` (rotative: 5 MB max, 3 generations retained). Failures (filesystem errors, panic, invalid markers) SHALL be logged with stack traces.
