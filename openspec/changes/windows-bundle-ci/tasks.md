...
- [x] 7.1 **Pre-release smoke-test** ✅ — `v0.0.0-winsmoke` run (3 rounds: r1+r2 caught two real pipeline bugs, r3 green). All 4 jobs green; release had `Nergal_0.4.1_x64-setup.exe` + `.exe.sig` + `Nergal_0.4.1_x64_en-US.msi`; `latest.json` had all 3 platforms (`linux-x86_64`, `darwin-aarch64`, `windows-x86_64` → `-setup.exe`, sig present); `isPrerelease: true` (v0.4.1 stayed `Latest`, no banner clobber). Pre-release + tag (local+remote) deleted after. **Bugs caught**: (1) the Windows updater artifact is `*-setup.exe(.sig)`, not `*-setup.nsis.zip`; (2) the shared `isMain` `file://${argv[1]}` idiom is false on Windows → `generate-latest-json` no-op'd → empty fragment → publish ENOENT (fixed in all 5 CI scripts).
- [x] 7.2 **Linux + macOS unchanged** ✅ — both jobs green in the smoke-test run; `latest.json` kept `linux-x86_64` + `darwin-aarch64` alongside the new `windows-x86_64`; all Linux/macOS assets (`.deb`/`.rpm`/`.AppImage`/`.dmg`/`.app.tar.gz`+sigs) present.
