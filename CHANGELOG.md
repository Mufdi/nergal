...
* Added a diagnostics report (copyable from Settings) and a status-bar notification center that keeps a history of past toasts
* Added localhost port management in the ports popover: inspect the process owning a listening port, free it (keyboard-navigable, stops a Docker container too) with the real binary shown for Chromium/Electron apps, and Docker Compose projects started in a session are stopped when you close Nergal
* Added a richer Activity drawer — per-tool duration and status, tool-call chain grouping, header stats, clickable touched files, clear-completed, filters, and full keyboard navigation
...
* Fixed the status bar deforming as activity text and rate-limit numbers changed — columns now hold stable widths, the center cluster stays centered, and the Claude/OpenAI incident badges no longer clip vertically
...
* Fixed two dead shortcuts/actions: "Open log file" in Settings and the cycle-layout shortcut (`Ctrl+Shift+I`)
...
* Fixed binding a task/issue — and spawning a worktree from its detail — not closing the detail modal
