> Checkboxes marked retroactively on 2026-06-05 (post-archive audit): shipped (BrowserHost/BrowserPanel/BrowserFloating live in the tree) and in daily use. Checkboxes were never tracked during implementation.
...
- [x] **1.1** Crear `src-tauri/src/browser.rs`:
...
- [x] **1.2** Implementar comando Tauri en `browser.rs`:
...
- [x] **1.3** Registrar comando en `src-tauri/src/commands.rs` (re-export) y wiring en `src-tauri/src/lib.rs`:
  - `mod browser;`
...
- [x] **1.5** Verify: `cd src-tauri && cargo clippy -- -D warnings && cargo test && cargo fmt --check`.

## Phase 2 — Frontend store + dock panel (~3h)

- [x] **2.1** Crear `src/stores/browser.ts`:
...
- [x] **2.2** Modificar `src/stores/rightPanel.ts`:
  - Añadir `"browser"` al `TabType` union
  - Push a `SINGLETON_TYPES` array
  - `PANEL_CATEGORY_MAP[browser] = "tool"`
- [x] **2.3** Crear `src/components/browser/BrowserToolbar.tsx`:
...
- [x] **2.4** Crear `src/components/browser/BrowserPanel.tsx`:
...
- [x] **2.5** Modificar `src/components/layout/RightPanel.tsx` para renderizar BrowserPanel cuando active tab es browser AND `browserModeAtom[sessionId] === "dock"`. Cuando mode === "floating", renderizar placeholder card "Browser está en modo flotante" + botón "Volver al dock".
- [x] **2.6** Wire `setupHookListeners` para `localhost:ports-changed` → actualiza `localhostPortsAtom`.
- [x] **2.7** Verify: `npx tsc --noEmit`.

## Phase 3 — Floating mode (~1h)

- [x] **3.1** Crear `src/components/browser/BrowserFloating.tsx`:
...
- [x] **3.2** **Mount-once integration** en `src/components/layout/Workspace.tsx`:
...
- [x] **3.3** Verify: manual test del mode-switch sin perder URL ni iframe state (scroll position, formulario half-filled, etc.).

## Phase 4 — Port detection UI (~1h)

- [x] **4.1** Modificar `src/components/layout/StatusBar.tsx`:
...
- [x] **4.2** Estilo de chips alineado con `DESIGN.md` tokens (compact, monospaced, hover state).
- [x] **4.3** Si lista vacía → no render chip placeholder (sin ruido visual).

## Phase 5 — Polish + docs (~1h)

- [x] **5.0** Shortcut "Open Browser" usa `Ctrl+Alt+B` (NO `Ctrl+Shift+W` — la W puede colisionar con muscle-memory de close-tab `Ctrl+W` y otros bindings de Linux WM).
- [x] **5.1** Añadir shortcut `Cmd+L` en `stores/shortcuts.ts`:
...
- [x] **5.2** Hookup del shortcut en `BrowserToolbar` (focus input ref + select-all del texto actual).
- [x] **5.3** Verificar que `ctrl+shift+0` (expand panel existente) funciona transparentemente con BrowserPanel — debería funcionar sin cambios porque BrowserPanel se monta dentro del slot del right panel.
- [x] **5.4** Update `DESIGN.md` con sección "Embedded browser pattern" (iframe + sandbox + mount-once con visibility toggling).
- [x] **5.5** Spec `live-preview-browser` en `openspec/changes/live-preview-browser/specs/live-preview-browser/spec.md` actualizado a iframe-first (ya hecho post-pivot, verificar).
...
- [x] **6.1** Full check:
  ...
  cd .. && npx tsc --noEmit
  ```
- [x] **6.2** Manual functional pass (los 9 pasos del Build contract → "Cómo verifico").
- [x] **6.3** Single-reviewer post-build: Claude evalúa diff y elige reviewer apropiado. Probable code-quality (no toca capabilities, sin security tag fuerte ya).
- [x] **6.4** Update `handoff/REVIEW.md` con outcomes.
- [x] **6.5** Suggest `/commit` al user (no auto-invoke).
- [x] **6.6** Suggest `/openspec-sync` al user para archive + update main specs.
