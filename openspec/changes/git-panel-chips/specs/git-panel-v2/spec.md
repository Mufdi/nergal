...
**Reason**: Replaced by the 5-chip strip. The 2-state segmented toggle is gone — chips give each surface a dedicated body without focus-switching ambiguity.
**Migration**: `gitSidebarModeAtom` and `GitSidebarMode` removed from `stores/git.ts`. The toggle UI in `GitPanel.tsx` is replaced by `<ChipStrip>`. Per-workspace persistence moves to `gitChipModeAtom`.

### Requirement: PR Viewer is opened as a document tab via openPrTabAction
**Reason**: PRs are a workspace-scoped list filter, not transient documents. The close-x button on the tab bar misrepresented the mental model. PRs now live inside the `PRs` chip in GitPanel.
**Migration**: `openPrTabAction`, `prTabId`, and the `pr` `TabType` are deleted. `PrViewer` is now embedded inside `PrsChip`. `RightPanel.tsx` no longer routes PR tabs.

### Requirement: PR list sidebar component
**Reason**: Subsumed by `PrsChip`'s picker mode.
**Migration**: `src/components/git/PrListSidebar.tsx` deleted.
