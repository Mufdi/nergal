...
The system SHALL store annotations in a SQLite table with fields: id, session_id, type, target, content, position_start, position_end, created_at. Annotations SHALL be scoped to the session that created them.
...
- **WHEN** frontend invokes `save_annotation` with type, target, content, position, and session_id
