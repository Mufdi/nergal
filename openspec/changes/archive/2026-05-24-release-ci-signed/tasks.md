> Checkboxes marked retroactively on 2026-06-05 (post-archive audit): in production since the v0.1.4 cut — CI builds, signs and publishes on every `v*` tag. Checkboxes were never tracked during implementation.

## 1. One-time prereqs (user-driven, documented)

- [x] 1.1 Generate Tauri signer keypair: `pnpm tauri signer generate -w ~/.tauri/cluihud-updater.key`. Set a strong password during the interactive prompt; this is needed for the GH secret.
- [x] 1.2 Open the keyfile, copy the **public key** line (after `untrusted comment: minisign public key ...`). It's a base64-encoded blob starting with `RW...`.
- [x] 1.3 Paste the pubkey into `src-tauri/tauri.conf.json` under `plugins.updater.pubkey`.
- [x] 1.4 Add GitHub repo secrets at https://github.com/Mufdi/nergal/settings/secrets/actions:
...
- [x] 1.5 Verify the keyfile is gitignored. Add `~/.tauri/` to a global `.gitignore` (or rely on it being outside the repo). Confirm with `git status` that the keyfile is invisible.

## 2. Tauri updater plugin (app side)

- [x] 2.1 Add to `src-tauri/Cargo.toml` under `[dependencies]`:
  ```toml
  tauri-plugin-updater = "2"
  ```
- [x] 2.2 Refresh `src-tauri/Cargo.lock`: `cd src-tauri && cargo check`.
- [x] 2.3 Initialize the plugin in `src-tauri/src/lib.rs::run()`:
  ```rust
  .plugin(tauri_plugin_updater::Builder::new().build())
  ```
  Add the `.plugin(...)` call to the existing `tauri::Builder` chain, alongside the other plugins.
- [x] 2.4 Configure the updater in `src-tauri/tauri.conf.json`:
  ```json
  "plugins": {
    "updater": {
      "active": true,
...
- [x] 2.5 Wire the existing About Update button (currently calls the `.deb` download flow) to branch on install source:
...
- [x] 2.6 Verify the app builds + runs locally with the new plugin: `cd src-tauri && cargo check && cargo clippy -- -D warnings && cd .. && pnpm tauri dev`. Open Settings → About and confirm the UI doesn't regress.

## 3. Release body extractor

- [x] 3.1 Refactor `scripts/release.mjs` to ensure `extractChangelogSection` is exported (already is per Change A — verify) and reusable.
- [x] 3.2 Create `scripts/extract-release-body.mjs`:
...
- [x] 3.3 Create `scripts/extract-release-body.test.mjs` with `node --test`:
  - Extracts body with header stripped
  - Missing section → exit 1 / null
  - Real-CHANGELOG-shape fixture
- [x] 3.4 Add `package.json` script: `"release:body": "node scripts/extract-release-body.mjs"`.
- [x] 3.5 Run `pnpm release:test` + `node --test scripts/extract-release-body.test.mjs` — all pass.
...
- [x] 4.2 Lint the workflow file with `actionlint` if installed locally (optional, GH Actions also validates on push).

## 5. Previous-release banner update

- [x] 5.1 Create `scripts/release-banner.tpl.md`:
  ```markdown
  ...
  {{ORIGINAL_BODY}}
  ```
- [x] 5.2 Create `scripts/update-previous-banner.mjs`:
...
- [x] 5.3 Add `package.json` script: `"release:banner": "node scripts/update-previous-banner.mjs"`.

## 6. Update Change A script's final message

- [x] 6.1 In `scripts/release.mjs`, replace the final stdout block:
  ```
...
- [x] 6.2 Update `--help` text to reflect the same change (drop "Build + GH release remain manual").

## 7. Documentation updates

- [x] 7.1 Update cluihud `CLAUDE.md` "Release commands" section:
...
- [x] 7.2 Update vault `Bug Workflow.md` step 6 — post-script the user just waits for CI; remove the manual build/release reminder.
- [x] 7.3 Update `bug-workflow/SKILL.md` rule about post-script behavior correspondingly.
- [x] 7.4 Update memory `project_v0_1_4_appimage_signing.md`:
...
- [x] 8.1 Unit tests pass: `pnpm release:test` + `node --test scripts/extract-release-body.test.mjs`.
- [x] 8.2 Local build with signing env vars set (from a local copy of the key — manually export them in shell, do NOT commit):
  ```bash
...
- [x] 8.3 CI smoke test: create a test branch, push a test tag like `v0.0.0-test1`. Verify the workflow:
  - Triggers
...
- [x] 8.4 First real release: cut v0.1.X via the full flow (Claude writes CHANGELOG section → `pnpm release patch` → wait for CI). Verify:
...
- [x] 8.5 Test rotation procedure: regenerate a fresh key (NOT in production, in a test environment), swap secrets, cut a test release, verify the OLD AppImage cannot verify the NEW update (expected — proves the rotation works).

## 9. Archive cleanup on close

- [x] 9.1 Move `Projects/nergal/follow-ups/release-automation.md` to `Records/Projects/nergal/release-automation follow-up.md`:
  ```bash
...
- [x] 9.2 Update or delete memory `project_v0_1_4_appimage_signing.md` based on whether anything is still load-bearing.
