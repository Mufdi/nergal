...
- [ ] 9.9 **PENDING (runtime, needs running app)**: live walk — solo-session turn produces no LLM call on Stop (only marker upsert); `get_session` on a dead yesterday-session regenerates from disk; deleting a session drops its summary+transcript rows.
